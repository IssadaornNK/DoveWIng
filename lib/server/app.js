...
app.post("/donations", async (req, res) => {
  const { name, details, amount, type, imgurl } = req.body;
  const authHeader = req.headers.authorization;
  if (!authHeader) {
    return res.status(401).json({ message: "Authorization header not found" });
  }
  ...
  jwt.verify(token, secretKey, (err, decoded) => {
    ...
    try {
      const userId = decoded.UserId;
      connection.query(
        "INSERT INTO Donation (name, details, date, amount, type, imgurl, userId) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [name, details, new Date(), amount, type, imgurl, userId],
        (err, result) => {
          if (err) {
